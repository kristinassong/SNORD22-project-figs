...
def calc_sno_coverage(df, df_int, region):
    ...
    if region != 'exon' and 'block_length' in df_cov.columns:
...
def compute_sno(args):
    snoid, df_up, df_exon, df_down, int_file, len_dict, norm_factor, nb_windows, nb_exons, outpath, snoid = args
    ...
    plot_coverage(sno_cov_up, sno_cov_exon, sno_cov_down, snoid + ' ' + snoid, outpath, pct_int)
...
def calc_coverage(df_up, df_exon, df_down, int_file, outdir, nb_windows, snolist, nb_threads, sno_dict):
    ...
    with Pool(nb_threads) as p:
        res = p.map(compute_sno, [[snoid, df_up, df_exon, df_down, int_file, len_dict, norm_factor,
                                   nb_windows, nb_exons, outpath, snoid] for snoid in snolist])
    for res_i in res:
...
def main():
    ...
    rbpfile = sys.argv[5] #csv file with chromosome and sno_id
    nb_threads = int(sys.argv[6]) # nb of threads to use in parallel
    
    rbplist=[]
    with open(rbpfile, 'r') as f:
        rbplist = f.readlines()

    with open(rbpfile, 'r') as f:
        rbplist = f.readlines()
    rbplist = [i.strip().split('\n')[0] for i in rbplist]
    ...
    df_sno = df_gtf[(df_gtf.feature == 'gene') & (df_gtf.gene_biotype == 'protein_coding')][['gene_id', 'gene_name']]
    ...
    calc_coverage(df_up, df_exon, df_down, int_file, outdir, nb_windows, rbplist, nb_threads, sno_dict)
